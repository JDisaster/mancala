...
public class MancalaModel {
    ...
    /**
     * Resotres the previous game state if the undo function is allowed
     */
    public void undo(){
        if(undoLastMove){
            System.out.println("Undo not allowed twice in a row");
        }
        if(currentPlayer.getUndoCount() >= 3){
            System.out.println("Undo limit reached for " + currentPlayer.getName());
        }
    ...
    public boolean isValidPit(int pitIndex){
        if(pitIndex == 6 || pitIndex == 13) {return false;}
     ...
     * @return the opposite pit index or -1 for invalid pit such as the Mancala of each player
     */
    ...
    /**
     * Makes a move based on the selected pit and checks if games over, invalid selection, captures, or to switch players or not
    ...
    public void makeMove(int pitIndex){
        ...
        //Grab all stones in selevted pit
        ...
        //Swtich players if last stone doesn't land in current players mancala
        if((currentPlayer == playerA && pitIndex != 6) || (currentPlayer == playerB && pitIndex != 13)){
            switchTurn();
        }
        //If one side of board is empty, move remaning stones in to their mancala sides
        if(isGameOver()){
            int sumA = 0;
            int sumB = 0;
